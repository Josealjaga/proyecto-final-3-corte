...
export const createQuotesDetail = async (req, res) => {
  const {quotes_id, products_id, quantity} = req.body;
  const sql = `INSERT INTO quotes_detail (quotes_id, products_id, quantity) VALUES ('${quotes_id}', '${products_id}', '${quantity}')`;
  connection.query(sql, (error, rows) => {
    if (error) {
      res.json(error);
    } else {
...
export const updateQuotesDetail = (req, res) => {
  const { id, quotes_id, products_id, quantity} = req.body;
  const sql = `UPDATE quotes_detail SET quotes_id = '${quotes_id}', products_id = '${products_id}', quantity = '${quantity}' WHERE id = '${id}'`;
  connection.query(sql, (error, rows) => {
    if (error) {
      res.json(error);
    } else {
